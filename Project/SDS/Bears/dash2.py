...
if submitted:
    ...
    new_df = new_df.join(pd.DataFrame(home_encoded, columns=['home_' + str(i) for i in range(home_encoded.shape[1])]))
    new_df = new_df.join(pd.DataFrame(away_encoded, columns=['away_' + str(i) for i in range(away_encoded.shape[1])]))
